...
public class UsuariosRegistradosDeserializador implements JsonDeserializer<RegistrosUsuarioResponse> {
    @Override
    public RegistrosUsuarioResponse deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        Gson gson = new Gson();
        RegistrosUsuarioResponse registrosUsuarioResponse =gson.fromJson(json, RegistrosUsuarioResponse.class);
        JsonArray registrousuarioResponseData = json.getAsJsonObject().getAsJsonArray(JsonKeys.MEDIA_RESPONSE_ARRAY);
